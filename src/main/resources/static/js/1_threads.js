...
function open_comment(event) {
  ...
  if (event.target.classList.contains("comment-content") == false) {
    return;
  }
  ...
  $.ajax({
    ...
    success: function (response) {
      ...
      for (let i = 0; i < slice_size; i++) {
        ...
        let formatted_createdAt = // 25-11-30 14:37
        ...
        let formatted_modifiedAt = // 25-11-30 14:37
          String(modifiedAt.getFullYear()).slice(2) + "-" +
          String(modifiedAt.getMonth() + 1).padStart(2, "0") + "-" +
          String(modifiedAt.getDate()).padStart(2, "0") + " " +
          String(modifiedAt.getHours()).padStart(2, "0") + ":" +
          String(modifiedAt.getMinutes()).padStart(2, "0");

        let temp_html = `
                    <li>
                        <div class="comment-main-level">
                            <div class="comment-avatar" data-bs-toggle="modal" data-bs-target="#exampleModal"
                                onclick="open_profile(event)">
                                <img src="https://cdn.pixabay.com/photo/2015/10/05/22/37/blank-profile-picture-973460_1280.png"
                                    alt="">
                            </div>
                            <div class="comment-box" data-post-id="${postId}" data-member-id="${memberId}"
                                onclick="open_comment(event)">
                                <div class="comment-head">
                                    <div style="display: flex; flex-direction: row; justify-content: space-between;">
                                        <div style="display: flex; flex-direction: column;">
                                            <h6 class="comment-name">
                                                <a href="#">${memberId} (${interested})</a>
                                            </h6>
                                            <div>
                                                <span style="padding-right: 5px;">post_id: ${postId},</span>
                                                <span style="padding-right: 5px;">comment: ${commentsCount}</span>
                                            </div>
                                        </div>
                                        <div style="display: flex; flex-direction: column;">
                                            <span style="padding-right: 5px;">created: ${formatted_createdAt}</span>
                                            <span style="padding-right: 5px;">modified: ${formatted_modifiedAt}</span>
                                        </div>
                                    </div>

                                    <i class="fa-solid fa-trash" data-post-id="${postId}" onclick="delete_post(event)"></i>
                                    <i class="fa-solid fa-pen" data-post-id="${postId}" onclick="open_modify(event)"
